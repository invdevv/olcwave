...
Управление профилями происходит на странице панели **Profiles**: создание, редактирование, удаление, поиск. При создании/редактировании панель проверяет, что YAML корректно разбирается (`yaml.safe_load`), но не выполняет глубокую проверку специфичных для OLCRTC полей - некорректная конфигурация OLCRTC всё равно будет сохранена и ошибка появится только при попытке запуска контейнера.

## Где хранятся профили

В PostgreSQL, а не на диске. YAML, который вы вставляете, является *шаблоном*. Конкретная конфигурация для каждого пользователя, которую реально запускает контейнер, создаётся динамически и записывается внутри контейнера в:
...
Когда для пользователя запрашивается подписка, для каждого профиля, для которого у пользователя ещё нет контейнера, панель выполняет следующее (`Subscriptions.profile_to_config`):
...
3. **Завершает room id** - берёт `room.id` и разбивает его по `/`:
...
## `crypto.key` - оставляйте пустым

В шаблоне профиля пишите:
...
**Не указывайте настоящий ключ.**

Панель сама генерирует уникальный ключ для каждого пользователя и каждого контейнера во время генерации конфигурации. Если прописать ключ вручную, все пользователи будут использовать один и тот же ключ - панель всё равно его перезапишет, поэтому в этом нет смысла.
